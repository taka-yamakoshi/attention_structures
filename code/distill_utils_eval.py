...
def eval_model_mask(tokenizer,model,device,head,text,max_length=None):
    ...
    for line in text:
        ...
        if "option_1" in head:
            ...
            assert " ".join(sent_1.split(" ")[:mask_span_start])==" ".join(sent_2.split(" ")[:mask_span_start])
            assert " ".join(sent_1.split(" ")[mask_span_end:])==" ".join(sent_2.split(" ")[mask_span_end:])
            assert " ".join(sent_1.split(" ")[mask_span_start:mask_span_end])!=" ".join(sent_2.split(" ")[mask_span_start:mask_span_end])
...
def calc_prob_mask(tokenizer, model, device, sent, mask_span):
    ...
    recon = tokenizer.decode(input_ids[start_id:end_id])
    assert recon.strip(" ,.!?;:").replace(" ","").lower()==target.strip(" ,.!?;:").replace(" ","").lower(), f"{recon}!={target}"
